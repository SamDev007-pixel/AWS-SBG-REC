...
export default function CloudOrbit() {
  ...
  // Tooltip direction for the hovered node
  const [activeTooltipDir, setActiveTooltipDir] = useState<"top" | "bottom" | "left" | "right">("top");
  ...
  useAnimationFrame((_t, delta) => {
    ...
    // If a node is hovered, rotation pauses and coordinates are static, so return early
    if (hovered) return;
  ...
  const handleEnter = useCallback(
    (node: NodeData, index: number) => {
      setHoveredId(node.id);
      hoveredIdRef.current = node.id;

      // Calculate direction once on hover
      ...
      setActiveTooltipDir(dir);
    },
    [radii, cx, cy, containerSize]
  );
