...
export const LearningGuidePanel: React.FC<LearningGuidePanelProps> = ({
  ...
  if (loading) {
    return (
      <div className="rounded-2xl bg-white/60 border border-slate-200/80 p-5 flex flex-col items-center justify-center min-h-[400px]">
        <div className="w-8 h-8 border-2 border-amber-200 border-t-[#FF9900] rounded-full animate-spin shadow-2xs" />
        <span className="text-[11px] uppercase font-bold text-slate-700 tracking-wider font-heading mt-3 animate-pulse">
          Loading Rules...
        </span>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl bg-white/[0.08] backdrop-blur-[20px] border border-white/20 p-5 flex flex-col gap-5 select-none w-full min-h-full">
      {/* Guidelines Header */}
