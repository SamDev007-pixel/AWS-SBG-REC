...
export default function ModuleLearningPage() {
  ...
  useEffect(() => {
    const loadModuleData = async () => {
      try {
        ...
        const activeModule = dbModules.find((m) => m.slug === moduleSlug);
        ...
        // Unlock validation — redirect locked modules
    ...
    return (
      <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-6 text-slate-100">
        <div className="max-w-xl w-full bg-rose-950/20 border-2 border-rose-500/30 rounded-[32px] p-8 shadow-2xl flex flex-col items-center text-center gap-6">
          <div className="w-16 h-16 rounded-2xl bg-rose-600 flex items-center justify-center text-white shadow-lg shadow-rose-600/30 animate-bounce">
  ...
  const handleNextSlide = () => {
    if (slideIndex + 1 < slides.length) setSlideIndex((prev) => prev + 1);
  };
  ...
  const handleOptionSelect = (optIndex: number) => {
    setUserAnswers((prev) => ({ ...prev, [currentQuizIndex]: optIndex }));
  };
  ...
  // ─── Main Render ──────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen w-full bg-white md:bg-gradient-to-br md:from-[#E0F2FE] md:via-[#F0F9FF] md:to-[#FFFFFF] flex items-start md:items-center justify-center p-0 md:p-4 font-sans select-none relative overflow-x-hidden text-slate-800">

      {/* Background Ambience */}
      <div className="hidden md:block absolute top-[10%] left-[5%] w-64 h-64 bg-cyan-300/10 rounded-full blur-[100px] pointer-events-none animate-pulse" />
      <div className="hidden md:block absolute bottom-[10%] right-[5%] w-96 h-96 bg-emerald-300/10 rounded-full blur-[120px] pointer-events-none animate-pulse" />
      ...
      <div className="w-full max-w-2xl bg-white border-0 md:border md:border-slate-200 rounded-none md:rounded-[32px] shadow-none md:shadow-2xl px-4 pt-4 pb-12 sm:px-6 sm:pt-6 sm:pb-8 md:p-8 flex flex-col min-h-screen md:min-h-[520px] relative z-10 justify-between">
