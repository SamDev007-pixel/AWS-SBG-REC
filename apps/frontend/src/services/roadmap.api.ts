...
export interface QuizQuestion {
  ...
  optionC: string;
  optionD: string;
  correctAnswer?: string; // Excluded for learner GET endpoints
  explanation: string;
  orderIndex: number;
}
...
export const modulesService = {
  getModules: async (topicId?: string): Promise<ModuleData[]> => {
    ...
    const res = await apiClient.get<ModuleData[]>('/roadmap/modules', { params });
    return res.data;
  },

  getModule: async (id: string): Promise<ModuleDetail> => {
    const res = await apiClient.get<ModuleDetail>(`/roadmap/modules/${id}`);
    return res.data;
  },

  getModuleBySlug: async (slug: string): Promise<ModuleDetail> => {
    const res = await apiClient.get<ModuleDetail>(`/roadmap/modules/slug/${slug}`);
    return res.data;
  },

  createModule: async (dto: CreateModuleDto): Promise<ModuleData> => {
    const res = await apiClient.post<ModuleData>('/roadmap/modules', dto);
    return res.data;
  },

  updateModule: async (id: string, dto: UpdateModuleDto): Promise<ModuleData> => {
    const res = await apiClient.patch<ModuleData>(`/roadmap/modules/${id}`, dto);
    return res.data;
  },

  deleteModule: async (id: string): Promise<{ success: boolean }> => {
    const res = await apiClient.delete<{ success: boolean }>(`/roadmap/modules/${id}`);
    return res.data;
  },

  reorderModules: async (ids: string[]): Promise<{ success: boolean }> => {
    const res = await apiClient.post<{ success: boolean }>('/roadmap/modules/reorder', { ids });
    return res.data;
  },
};
...
export const slidesService = {
  getSlides: async (moduleId: string): Promise<LearningSlide[]> => {
    const res = await apiClient.get<LearningSlide[]>(`/roadmap/modules/${moduleId}/slides`);
    return res.data;
  },

  syncSlides: async (moduleId: string, slides: LearningSlide[]): Promise<LearningSlide[]> => {
    const res = await apiClient.put<LearningSlide[]>(`/roadmap/modules/${moduleId}/slides`, { slides });
    return res.data;
  },
  ...
  uploadImage: async (file: File): Promise<{ url: string }> => {
    ...
    const res = await apiClient.post<{ url: string }>('/roadmap/uploads/image', formData, {
...
export const questionsService = {
  getQuestions: async (moduleId: string): Promise<QuizQuestion[]> => {
    const res = await apiClient.get<QuizQuestion[]>(`/roadmap/modules/${moduleId}/questions`);
    return res.data;
  },

  syncQuestions: async (moduleId: string, questions: QuizQuestion[]): Promise<QuizQuestion[]> => {
    const res = await apiClient.put<QuizQuestion[]>(`/roadmap/modules/${moduleId}/questions`, { questions });
    return res.data;
  },
};
...
export const progressService = {
  getMyProgress: async (): Promise<UserProgress> => {
    const res = await apiClient.get<UserProgress>('/roadmap/progress/me');
    return res.data;
  },

  getModuleProgress: async (moduleId: string): Promise<ModuleProgress> => {
    const res = await apiClient.get<ModuleProgress>(`/roadmap/modules/${moduleId}/progress`);
    return res.data;
  },

  submitQuizAttempt: async (moduleId: string, dto: QuizAttemptDto): Promise<QuizAttemptResult> => {
    const res = await apiClient.post<QuizAttemptResult>(`/roadmap/modules/${moduleId}/quiz/attempt`, dto);
    return res.data;
  },

  getQuizReview: async (moduleId: string): Promise<any> => {
...
export const learningService = {
  getTopicList: async (): Promise<TopicSummary[]> => {
    const res = await apiClient.get<{ topics: TopicSummary[] }>('/roadmap/learning/topics');
    return res.data.topics;
  },

  getTopicDetail: async (slug: string): Promise<LearningTopicDetail> => {
    const res = await apiClient.get<LearningTopicDetail>(`/roadmap/learning/topics/${slug}`);
    return res.data;
  },
...
export const topicsService = {
  getTopics: async (): Promise<TopicData[]> => {
    const res = await apiClient.get<TopicData[]>('/roadmap/topics');
    return res.data;
  },

  getTopic: async (id: string): Promise<TopicData> => {
    const res = await apiClient.get<TopicData>(`/roadmap/topics/${id}`);
    return res.data;
  },

  createTopic: async (dto: CreateTopicDto): Promise<TopicData> => {
    const res = await apiClient.post<TopicData>('/roadmap/topics', dto);
    return res.data;
  },

  updateTopic: async (id: string, dto: UpdateTopicDto): Promise<TopicData> => {
    const res = await apiClient.patch<TopicData>(`/roadmap/topics/${id}`, dto);
    return res.data;
  },

  deleteTopic: async (id: string): Promise<{ success: boolean }> => {
    const res = await apiClient.delete<{ success: boolean }>(`/roadmap/topics/${id}`);
    return res.data;
  },

  reorderTopics: async (ids: string[]): Promise<{ success: boolean }> => {
    const res = await apiClient.post<{ success: boolean }>('/roadmap/topics/reorder', { ids });
    return res.data;
  },
};
...
export const learnersService = {
  getLearners: async (): Promise<LearnerSummary[]> => {
    const res = await apiClient.get<LearnerSummary[]>('/roadmap/learners');
    return res.data;
  },
};
